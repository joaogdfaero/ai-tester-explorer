...
Before any testing, spend 2-3 minutes USING the app the way a customer would. Not testing -- USING. If the scope was provided and/or mentioned by the user, focus only on the "In Scope" areas from `data\scope`. Ignore everything that is in the "Out Of Scope" area if provided.

Explore the app as if you just downloaded it and want to use it yourself. Don’t worry about finding any bugs right now. You may stumble upon them, but this phase is really just about getting used to the app's layout and logic. Jot down anything you find that you want to explore further later. 
...
- E-commerce: "BrowsTarget Vulnerabilities: Once you get a feel for the app, start going back to the areas that interested you or that you suspect might be vulnerable. Knowing where an app is vulnerable comes with experience, and don’t worry if you don’t have that experience yet, because you are about to get some! e -> Add to cart -> Checkout -> Confirm order -> Verify in order history"
