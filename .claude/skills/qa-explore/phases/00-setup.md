...
- **scope**: In and out of scope areas (optional -- auto-detect if not specified)
...
Resolve target config from `data/targets/` if available. Read domain config from `data/domains/`. If the target config contains a `known_errors` section, load it now — it will be used in Phase 3 to suppress baseline noise and highlight only new errors.

## Step 2: Scope
If specified or mentioned by the user, always read the testing scope from `data/scope/<scope-file>.md`.

## Step 3: Load Context (if provided)
...
## Step 7: Optional — Bootstrap Playwright Test Agents (opt-in)
